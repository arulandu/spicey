mod structs;
pub use structs::{NgVecInfoAll, NgVecValuesAll, VecValuesAll, VecInfoAll};
...
impl NgSpice {
    ...
    pub fn init<T: NgSpiceManager>(&self, manager: Option<T>) -> Result<(), String> {
        unsafe extern "C" fn cbw_send_char<T: NgSpiceManager>(
            msg: *const c_char,
            id: c_int,
            user: *const c_void,
        ) -> c_int {
            unsafe {
                <T as NgSpiceManager>::send_char(
                    &mut *(user as *mut T),
        ...
        unsafe extern "C" fn cbw_send_stat<T: NgSpiceManager>(
            msg: *const c_char,
            id: c_int,
            user: *const c_void,
        ) -> c_int {
            unsafe {
                <T as NgSpiceManager>::send_stat(
                    &mut *(user as *mut T),
                    std::ffi::CStr::from_ptr(msg).to_str().unwrap().to_owned(),
                    id,
                );
        ...
        unsafe extern "C" fn cbw_controlled_exit<T: NgSpiceManager>(
            status: c_int,
            immediate: bool,
            exit_on_quit: bool,
            id: c_int,
            ...
            unsafe {
                <T as NgSpiceManager>::controlled_exit(
                    &mut *(user as *mut T),
                    status,
                    immediate,
                    exit_on_quit,
        ...
        unsafe extern "C" fn cbw_send_data<T: NgSpiceManager>(
            pvecvaluesall: *const NgVecValuesAll,
            count: c_int, // redundant for NgVecValuesAll.veccount
            id: c_int,
            user: *const c_void,
        ) -> c_int {
            unsafe {
                let x = VecValuesAll::from(&*pvecvaluesall);
                <T as NgSpiceManager>::send_data(&mut *(user as *mut T), x, count, id);
            }
            0
        }
        unsafe extern "C" fn cbw_send_init_data<T: NgSpiceManager>(
            ...
            unsafe {
                let x = VecInfoAll::from(&*pvecinfoall);
                <T as NgSpiceManager>::send_init_data(&mut *(user as *mut T), x, id);
            }
            0
        }
        unsafe extern "C" fn cbw_bgthread_running<T: NgSpiceManager>(
            finished: bool,
            id: c_int,
            user: *const c_void,
        ) -> c_int {
        ...
        let ret = unsafe {
            let ngSpice_Init = self.get_symbol::<extern "C" fn(
    ...
    pub fn command(&self, s: &str) -> Result<(), String> {
        let ret = unsafe {
            let ngSpice_Command =
                self.get_symbol::<extern "C" fn(*const c_char) -> c_int>(b"ngSpice_Command\0");
    ...
    pub fn load(&self, c: &Ast) -> Result<(), String> {
        for mut line in c.netlist().split('\n') {
            line = line.trim();
            if line.is_empty() { continue; }

            let cmd = format!("circbyline {}", line);
            self.command(&cmd)?
        }
        Ok(())
    }
    ...
    Self: Sized,
{
    ...
    fn send_data(&mut self, pvecvaluesall: VecValuesAll, count: i32, id: i32) {}
    fn send_init_data(&mut self, pvecinfoall: VecInfoAll, id: i32) {}
